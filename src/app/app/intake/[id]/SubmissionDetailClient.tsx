...
export default function SubmissionDetailClient({ submission }: { submission: Submission }) {
  ...
  return (
    <div className="p-6 max-w-5xl">
      {/* Back */}
      <a href="/app/intake" className="flex items-center gap-2 text-sm mb-6 transition-colors hover:text-white"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Header card */}
          <div className="rounded-2xl p-6" style={{ background: T.navy800, border: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h1 className="text-xl font-bold text-white">{submission.clientName ?? "Unknown Client"}</h1>
                </div>
          ...
          {/* Answers */}
          <div className="rounded-2xl overflow-hidden" style={{ background: T.navy800, border: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="px-5 py-3.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <h2 className="font-semibold text-white text-sm">Client Answers</h2>
            </div>
              ...
              {answerEntries.length === 0 ? (
                ...
                answerEntries.map(([key, value]) => (
                  <div key={key} className="px-5 py-3 flex gap-4">
                    ...
                    <span className="text-sm text-white flex-1 whitespace-pre-wrap">
          ...
          {submission.documents.length > 0 && (
            <div className="rounded-2xl overflow-hidden" style={{ background: T.navy800, border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="px-5 py-3.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <h2 className="font-semibold text-white text-sm">Documents ({submission.documents.length})</h2>
              </div>
              <div className="p-4 space-y-2">
                {submission.documents.map((doc) => (
                  <div key={doc.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                    style={{ background: T.navy700, border: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div className="space-y-5">
          {/* Linked matter */}
          {submission.matter && (
            <div className="rounded-2xl p-5" style={{ background: "rgba(45,212,160,0.07)", border: "1px solid rgba(45,212,160,0.2)" }}>
              <p className="text-xs font-semibold mb-2" style={{ color: T.teal }}>✓ Matter Created</p>
              <p className="text-sm font-semibold text-white">{submission.matter.title}</p>
              <p className="text-xs mt-1" style={{ color: T.gray500 }}>{submission.matter.stage}</p>
            </div>
          )}

          {/* Review notes */}
          <div className="rounded-2xl p-5" style={{ background: T.navy800, border: "1px solid rgba(255,255,255,0.06)" }}>
            <h3 className="font-semibold text-white text-sm mb-3">Review Notes</h3>
            <textarea value={reviewNotes} onChange={(e) => setReviewNotes(e.target.value)}
              rows={4} placeholder="Add notes for the attorney…"
              className="w-full rounded-xl px-4 py-3 text-sm resize-none outline-none"
              style={{ background: T.navy700, border: "1px solid rgba(255,255,255,0.08)", color: "#fff" }}
            />
